...
TEST(strlen_returns_correct_length, String_fixture) {
    ...
    ASSERT_EQ(strlen(TEST->str), TEST->length, "%lu");
}
 ...
 * identical.  The final assertion, ASSERT_EQ, is slightly more complicated.  It
 * checks that the two specified values are equal, but also requires a printf-
 * style format code as a final argument.  This argument controls how the values
 * will be displayed if the assertion fails, providing you a limited degree of
 * customisation of the reports generated by your test suites.  More
 * practically, this argument is required because the C preprocessor lacks any
 * form of true metaprogramming, so we have no way to determine how to format
 * the values being compared in assertions.
 *
      ...
      { T_ str = "One more parameter set"; T_ length = 22; })
{
    ...
    ASSERT_EQ(strlen(T_ str), T_ length, "%lu");
}
...
TEST(Copy_to_dynamic_string, Custom_lifecycle_fixture) {
    ...
    ASSERT_EQ(strlen(T_ str), (size_t) 28, "%lu");
}
